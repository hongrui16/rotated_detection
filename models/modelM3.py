...
class ModelM3(nn.Module):
    def __init__(self, pretrained = True, weight_filepath = None):
    ...
    def get_logits(self, x):
        ...
        conv10 = self.avgpool(conv10) #[B, 176, 8, 8]
        flat1 = torch.flatten(conv10.permute(0, 2, 3, 1), 1)
        logits = self.fc1_bn(self.fc1(flat1))
        return logits, conv10
    
    def forward(self, x):
        logits, conv10 = self.get_logits(x)
        return F.log_softmax(logits, dim=1), conv10
    ...
    def _load_pretrained_model(self, weight_filepath = None):
        if weight_filepath is None:
            return
        pretrain_dict = torch.load(weight_filepath)
        model_dict = {}
